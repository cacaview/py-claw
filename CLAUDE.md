...
- 2026-09-15：WebSearch 工具接入真实联网搜索——新增 `tools/web_search_backends.py`，提供免 API key 的多引擎搜索（Bing / DuckDuckGo / Baidu，设计参考 open-webSearch MCP server），`engine="auto"` 按序降级、单引擎失败记入 failures 不中断、`/ck/a` 与 baidu `/link` 跳转链接解码为真实 URL、支持 `allowed_domains`/`blocked_domains` 域过滤；`WebSearchTool` 输入新增 `engine`/`max_results`（1-20），全失败时返回结构化 error 而非抛异常；全程标准库实现（urllib + html.parser），无新依赖；新增 55 个单元测试（test_web_search_backends.py + test_web_search_tool.py），并将 test_tools_runtime.py 中 4 个 WebSearch 测试改为离线 patch；已通过真实端到端验证（--print stream-json 工具调用 + usage.webSearchRequests 计数 + TUI 权限对话框与工具结果渲染）
