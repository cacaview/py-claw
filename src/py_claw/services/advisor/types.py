...
class AdvisorRedactedContent(TypedDict):
    """Advisor redacted result content."""
    type: Literal['advisor_redacted_result']
    encrypted_content: str


class AdvisorErrorContent(TypedDict):
    """Advisor tool result error content."""
...
class AdvisorToolResultBlock(TypedDict):
    ...
    tool_use_id: str
    content: AdvisorResultContent | AdvisorRedactedContent | AdvisorErrorContent
...
@dataclass
class AdvisorConfig:
    """Advisor configuration from GrowthBook."""
    enabled: bool = False
...
@dataclass
class ExperimentAdvisorModels:
    """Models from experiment config."""
    base_model: str
    advisor_model: str
